...
<body class="bg-bg text-text transition-colors duration-200 min-h-screen flex flex-col font-sans tracking-tight">
    ...
    <header class="sticky top-0 z-50 w-full backdrop-blur-md bg-bg/80 border-b border-border shadow-sm transition-colors duration-300">
        <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between py-1.5" aria-label="Main Navigation">
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                <button id="theme-toggle" type="button" class="text-muted hover:bg-surface focus:outline-none focus-visible:ring-4 focus-visible:ring-accent/50 rounded-lg p-0.5 transition-all border border-transparent flex items-center justify-center" aria-label="Toggle Dark Mode">
                ...
                <?php if (isset($_SESSION['user_id'])): ?>
                    ...
                    <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'Admin'): ?>
                        <a href="<?php echo BASE_URL; ?>admin/dashboard.php" class="text-sm font-semibold text-text hover:text-red-600 focus:outline-none focus:ring-4 focus:ring-red-300 rounded px-2 py-0.5 transition-colors whitespace-nowrap leading-none flex items-center">Admin Panel</a>
                    <?php else: ?>
                        <a href="<?php echo BASE_URL; ?>jobs.php" class="text-sm font-medium text-text hover:text-accent focus:outline-none focus:ring-4 focus:ring-accent rounded px-2 py-0.5 transition-colors whitespace-nowrap leading-none flex items-center">Available Roles</a>
                    <?php endif; ?>

                    <div class="h-4 w-px bg-border hidden sm:block"></div>

                    <a href="<?php echo BASE_URL; ?>actions/logout.php" class="text-sm font-medium text-muted hover:text-text focus:outline-none focus:ring-4 focus:ring-border rounded px-2 py-0.5 transition-colors whitespace-nowrap leading-none flex items-center">Log Out</a>

                <?php else: ?>
                    <a href="<?php echo BASE_URL; ?>login.php" class="text-sm font-semibold text-text hover:text-accent focus:outline-none focus-visible:ring-4 focus-visible:ring-accent rounded px-2 py-0.5 transition-all whitespace-nowrap leading-none flex items-center">Sign In</a>
                    <a href="<?php echo BASE_URL; ?>register.php" class="bg-accent text-white px-5 py-2.5 rounded-lg font-semibold hover:bg-accent-hover active:scale-95 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-accent/50 whitespace-nowrap leading-none flex items-center">Join Platform</a>
            </div>
        </nav>
    </header>
